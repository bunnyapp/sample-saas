...
const registerHandler: RequestHandler = async (req, res) => {
  ...
  if (
    !process.env.BUNNY_BASE_URL ||
    !process.env.BUNNY_ACCESS_TOKEN ||
        ...
        try {
          ...
          if (!subscription || !subscription.id) {
            ...
            await createEvent(
              ...
              "subscription_create",
              "error",
              "Invalid subscription response from Bunny"
            );
            return res.status(500).json({
    ...
    db.get<AccountLimits>(
      ...
      (err, limits) => {
        if (err) {
          return res
            .status(500)
            .json({ error: "Error fetching account limits" });
        }
          ...
          db.run(
            ...
            (err) => {
              if (err) {
                return res
                  .status(500)
                  .json({ error: "Error setting default limits" });
              }
            ...
            db.get<Todo>(
              ...
              (err, todo) => {
                if (err) {
                  return res
                    .status(500)
                    .json({ error: "Error fetching created todo" });
                }
                res.json(todo);
              }
            );
